...
import { esc, h, errorText, formatBytes, plural, actionSheet, confirmSheet, toast } from "./ui.js?v=63";
import { isBlocked } from "./files.js?v=63";
import { putPending, MAX_BYTES } from "./pending.js?v=63";
import { mountWallpaperNote } from "./wallpapers.js?v=63";
...
async function ensureAccount(form, fail) {
  ...
  const session = await import("./session.js?v=63");
  await session.ensureAuth();
  const { ensureVerified } = await import("./verify.js?v=63");
    ...
    k.isHost ? {
      ...
      run: async () => {
        ...
        try {
          const session = await import("./session.js?v=63");
          await session.ensureAuth();
          const api = await import("./api.js?v=63");
...
if (accountEmail()) {
  import("./session.js?v=63").then((m) => m.syncSpaces()).then(() => {
    drawSpacesLink();
