...
import { esc, h, errorText, formatBytes, plural, actionSheet, confirmSheet, toast } from "./ui.js?v=91";
import { isBlocked, FILE_MAX } from "./files.js?v=91";
import { putPending, MAX_BYTES } from "./pending.js?v=91";
import { mountWallpaperNote } from "./wallpapers.js?v=91";
import { mountClaim } from "./claim-fx.js?v=91";
...
async function ensureAccount(form, fail) {
  ...
  const session = await import("./session.js?v=91");
  await session.ensureAuth();
  const { ensureVerified } = await import("./verify.js?v=91");
    ...
    k.isHost ? {
      ...
      run: async () => {
        ...
        try {
          const session = await import("./session.js?v=91");
          await session.ensureAuth();
          const api = await import("./api.js?v=91");
...
if (accountEmail()) {
  import("./session.js?v=91").then((m) => m.syncSpaces()).then(() => {
    drawSpacesLink();
