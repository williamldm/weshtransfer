...
import { esc, h, errorText, formatBytes, plural, actionSheet, confirmSheet, toast } from "./ui.js?v=51";
import { isBlocked } from "./files.js?v=51";
import { putPending, MAX_BYTES } from "./pending.js?v=51";
import { mountWallpaperNote } from "./wallpapers.js?v=51";
...
async function ensureAccount(form, fail) {
  ...
  const session = await import("./session.js?v=51");
  await session.ensureAuth();
  const { ensureVerified } = await import("./verify.js?v=51");
    ...
    k.isHost ? {
      ...
      run: async () => {
        ...
        try {
          const session = await import("./session.js?v=51");
          await session.ensureAuth();
          const api = await import("./api.js?v=51");
...
if (accountEmail()) {
  import("./session.js?v=51").then((m) => m.syncSpaces()).then(() => {
    drawSpacesLink();
