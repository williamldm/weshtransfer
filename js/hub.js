...
import { esc, h, errorText, formatBytes, plural, actionSheet, confirmSheet, toast } from "./ui.js?v=62";
import { isBlocked } from "./files.js?v=62";
import { putPending, MAX_BYTES } from "./pending.js?v=62";
import { mountWallpaperNote } from "./wallpapers.js?v=62";
...
async function ensureAccount(form, fail) {
  ...
  const session = await import("./session.js?v=62");
  await session.ensureAuth();
  const { ensureVerified } = await import("./verify.js?v=62");
    ...
    k.isHost ? {
      ...
      run: async () => {
        ...
        try {
          const session = await import("./session.js?v=62");
          await session.ensureAuth();
          const api = await import("./api.js?v=62");
...
if (accountEmail()) {
  import("./session.js?v=62").then((m) => m.syncSpaces()).then(() => {
    drawSpacesLink();
