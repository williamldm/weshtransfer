...
import { esc, h, errorText, formatBytes, plural, actionSheet, confirmSheet, toast } from "./ui.js?v=42";
import { isBlocked } from "./files.js?v=42";
import { putPending, MAX_BYTES } from "./pending.js?v=42";
import { mountWallpaperNote } from "./wallpapers.js?v=42";
...
async function ensureAccount(form, fail) {
  ...
  const session = await import("./session.js?v=42");
  await session.ensureAuth();
  const { ensureVerified } = await import("./verify.js?v=42");
    ...
    k.isHost ? {
      ...
      run: async () => {
        ...
        try {
          const session = await import("./session.js?v=42");
          await session.ensureAuth();
          const api = await import("./api.js?v=42");
...
if (accountEmail()) {
  import("./session.js?v=42").then((m) => m.syncSpaces()).then(() => {
    drawSpacesLink();
