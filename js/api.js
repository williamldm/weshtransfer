...
import { sb, q, invoke, requireClient } from "./db.js?v=89";
...
export function listProjects(spaceId) {
  return q(db().from("projects")
    .select("id, title, bpm, musical_key, last_activity_at, created_at, created_by, position, creator:participants(pseudo), files(id, kind, status, version_no, approved_at, original_name, mime_type, uploaded_by, duration_sec, label, created_at)")
    .eq("space_id", spaceId)
    .eq("archived", false)
